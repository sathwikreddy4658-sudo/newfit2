...
app.post("/send-email", async (req, res) => {
    try {
        const { orderId, customerEmail, customerName, totalPrice, orderItems, address, paymentMethod, emailType = "confirmation", trackingNumber, carrierName, estimatedDeliveryDate, } = req.body;
        // Validate required fields
        ...
        if (emailType === "confirmation") {
            ...
            const itemsHtml = (Array.isArray(orderItems)
                ? orderItems
                : [])
                .map((item) => `<tr><td style="padding:10px;border-bottom:1px solid #ddd;">${item.name}</td><td style="padding:10px;border-bottom:1px solid #ddd;">₹${item.price}</td></tr>`)
                .join("");
            emailHtml = `
      <html>
